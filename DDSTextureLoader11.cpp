                             ...
                             _Outptr_opt_ ID3D11ShaderResourceView **textureView) noexcept
{
    ...
    if (autogen)
    {
        ...
        hr = CreateD3DResources(d3dDevice, resDim, width, height, depth, 0, arraySize, format, usage,
                                ...
                                miscFlags | D3D11_RESOURCE_MISC_GENERATE_MIPS, forceSRGB, isCubeMap, nullptr, &tex,
                                textureView);
        if (SUCCEEDED(hr))
        {
            size_t numBytes = 0;
    ...
    else
    {
        ...
        if (SUCCEEDED(hr))
        {
            hr = CreateD3DResources(d3dDevice, resDim, twidth, theight, tdepth, mipCount - skipMip, arraySize, format,
                                    usage, bindFlags, cpuAccessFlags, miscFlags, forceSRGB, isCubeMap, initData.get(),
                                    texture, textureView);
            ...
            if (FAILED(hr) && !maxsize && (mipCount > 1))
            {
                ...
                if (SUCCEEDED(hr))
                {
                    hr = CreateD3DResources(d3dDevice, resDim, twidth, theight, tdepth, mipCount - skipMip, arraySize,
                                            format, usage, bindFlags, cpuAccessFlags, miscFlags, forceSRGB, isCubeMap,
                         ...
                         _In_opt_ ID3D11ShaderResourceView **textureView) noexcept
{
    ...
    if (texture || textureView)
    {
        ...
        if (result > 0)
        {
            ...
            if (texture && *texture)
            {
                (*texture)->SetPrivateData(WKPDID_D3DDebugObjectName, static_cast<UINT>(strnlen_s(pstrName, MAX_PATH)),
                                           pstrName);
            }
            ...
            {
                (*textureView)
                    ->SetPrivateData(WKPDID_D3DDebugObjectName, static_cast<UINT>(strnlen_s(pstrName, MAX_PATH)),
                                     pstrName);
            }
        }
    }
                                                                   ...
                                                                   DDS_ALPHA_MODE *alphaMode) noexcept
{
    return CreateDDSTextureFromMemoryEx(d3dDevice, nullptr, ddsData, ddsDataSize, maxsize, D3D11_USAGE_DEFAULT,
                                        D3D11_BIND_SHADER_RESOURCE, 0, 0, false, texture, textureView, alphaMode);
}
                                                                     ...
                                                                     DDS_ALPHA_MODE *alphaMode) noexcept
{
    ...
    hr = CreateTextureFromDDS(d3dDevice, d3dContext, header, bitData, bitSize, maxsize, usage, bindFlags,
                              cpuAccessFlags, miscFlags, forceSRGB, texture, textureView);
    if (SUCCEEDED(hr))
    {
        if (texture && *texture)
        {
